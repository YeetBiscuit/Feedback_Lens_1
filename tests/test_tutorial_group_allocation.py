...
class TutorialGroupRouteTests(unittest.TestCase):
    ...
    def test_unit_page_and_tutorial_group_routes(self):
        ...
        setup_position = page.data.index(b"Tutorial group setup")
        groups_position = page.data.index(b"<h2>Tutorial Groups</h2>")
        staff_position = page.data.index(b"<h2>Unit staff</h2>")
        self.assertLess(setup_position, groups_position)
        self.assertLess(groups_position, staff_position)
        self.assertIn(b"Import students and groups", page.data)
        self.assertIn(b"Tutorial Groups", page.data)
        self.assertIn(b"Import students from Applied spreadsheet", page.data)
        self.assertIn(b"Assign tutors", page.data)
        self.assertIn(b"Assign tutors from CSV", page.data)
        self.assertIn(b"Assign a tutor", page.data)
        self.assertNotIn(b"Teaching team", page.data)
