...
def _prompt_optional_int(label: str) -> int | None:
    while True:
        ...
        try:
            ...
            print(f"Invalid integer for {label}. Please enter a whole number or leave blank.")


def get_units(conn: sqlite3.Connection) -> list[sqlite3.Row]:
    return conn.execute(
        """
        SELECT unit_id, unit_code, unit_name, semester, year
        FROM units
        ORDER BY unit_code, year, semester, unit_id
        """
    ).fetchall()


def pick_unit(conn: sqlite3.Connection) -> sqlite3.Row | None:
    units = get_units(conn)
    if not units:
        print("No units found. Add a unit first.")
        return None

    print("\nAvailable units:")
    for index, unit in enumerate(units, start=1):
        semester = unit["semester"] or "No semester"
        year = unit["year"] if unit["year"] is not None else "No year"
        print(
            f"{index}. [{unit['unit_id']}] {unit['unit_code']} - "
            f"{unit['unit_name']} ({semester}, {year})"
        )

    choice = input("Choose unit by number or unit_id: ").strip()
    if not choice:
        return None

    if choice.isdigit():
        numeric_choice = int(choice)
        if 1 <= numeric_choice <= len(units):
            return units[numeric_choice - 1]

        for unit in units:
            if unit["unit_id"] == numeric_choice:
                return unit
...
def create_unit(conn: sqlite3.Connection) -> None:
    print("\nAdd unit")
    unit_code = _prompt_required("Unit code")
    unit_name = _prompt_required("Unit name")
    semester = _prompt_optional("Semester [optional]")
    year = _prompt_optional_int("Year [optional]")

    cur = conn.execute(
        """
        INSERT INTO units (unit_code, unit_name, semester, year)
        VALUES (?, ?, ?, ?)
        """,
    ...
    print(
        f"Unit created: unit_id={cur.lastrowid}, "
        f"unit_code={unit_code}, unit_name={unit_name}."
    )


def create_assignment(conn: sqlite3.Connection) -> None:
    print("\nAdd assignment")
    unit = pick_unit(conn)
    if unit is None:
        return

    assignment_name = _prompt_required("Assignment name")
    assignment_type = _prompt_optional("Assignment type [optional]")
    description = _prompt_optional("Description [optional]")
    due_date = _prompt_optional("Due date [optional, e.g. 2026-05-01]")

    cur = conn.execute(
        """
        INSERT INTO assignments
            (unit_id, assignment_name, assignment_type, description, due_date)
        VALUES (?, ?, ?, ?, ?)
        """,
        (
            unit["unit_id"],
    ...
    print(
        f"Assignment created: assignment_id={cur.lastrowid}, "
        f"unit_id={unit['unit_id']}, assignment_name={assignment_name}."
    )
...
5. View table rows
6. Insert row
7. Update row by primary key
8. Delete row by primary key
9. Run custom SQL
0. Exit
"""
    )
...
def confirm_reset() -> bool:
    print("\nWARNING: This will permanently delete all SQLite data and all vectors in chromadb/.")
