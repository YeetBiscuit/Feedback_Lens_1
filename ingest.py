...
from txt_reader import read_transcript
from chunking import chunk_pages
from embedding import embed_and_store, MODEL_NAME
...
def _extract_pages(file_path: Path) -> list[dict]:
    ...
    if reader is None:
        raise ValueError(
            f"Unsupported file type '{suffix}'. Supported types: {', '.join(_READERS)}"
        )
    return reader(str(file_path))


def _build_collection_name(unit_code: str, year: int | None, semester: str | None) -> str:
    """
...
def ingest_material(
    ...
    cleaned_text = raw_text  # preprocessing placeholder
